...
cat(sprintf(
    "Data: n=%d, p1=%d, p2=%d, groups=%d\n",
    nrow(X1_full), ncol(X1_full), ncol(X2_full), length(unique(labels))
))
cat(sprintf(
    "Anchor set: %d subjects (%.0f%% complete)\n\n",
...
X1_miss <- X1_full
X1_miss[mask[, "rnaseq"], ] <- NA
X2_miss <- X2_full
X2_miss[mask[, "methylation"], ] <- NA
...
check_orth <- function(W, B, name) {
    ...
    cat(sprintf(
        "  %s: W^T B W diagonal range [%.4f, %.4f], max off-diag %.2e\n",
...
cat(sprintf(
    "  Latent Z1: within-group var=%.4f, between-group var=%.4f\n",
