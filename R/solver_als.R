...
woven_als <- function(X_list, anchor_idx, Y,
  ...
  if (is.null(L_list_precomp) || is.null(Omega_list_precomp)) {
    if (verbose) cat("  Building Laplacians...\n")
    L_list <- if (!is.null(L_list_precomp)) L_list_precomp else {
      lapply(seq_len(V), function(v) {
        Xref <- if (!is.null(X_list_full)) X_list_full[[v]] else X_list[[v]]
        build_laplacian(Xref, k = k_nn)
      })
    }
    Omega_list_local <- if (!is.null(Omega_list_precomp)) Omega_list_precomp else {
      lapply(seq_len(V), function(v) {
  ...
  if (verbose) cat("  Computing B matrices...\n")
  ...
  b_orth <- function(W, B) {
    ...
    if (!is.null(R)) return(t(backsolve(R, t(W))))
