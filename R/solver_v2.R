...
#' @param Omega1_precomp,Omega2_precomp optional precomputed Omega (graph smoothing) matrices
#' @param L1_precomp,L2_precomp optional precomputed sample Laplacians
#' @param XtX1_precomp,XtX2_precomp optional precomputed X^T X matrices
...
#' fit <- woven_v2(X1, X2, anchor_idx = anchor_idx, Y = Y, K = K)
#' dim(fit$W1)
#' @export
woven_v2 <- function(X1, X2, anchor_idx, Y,
    ...
    if (verbose) {
        message(sprintf(
            "WOVEN V=2 | n=%d, n_a=%d, p1=%d, p2=%d, K=%d, gamma_y=%.2f, alpha=[%.3f,%.3f]",
    ...
    #  Matrix square roots
    if (verbose) message("  Computing matrix square roots...")
    ...
    if (verbose) message("  Computing SVD of P...")
