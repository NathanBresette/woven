...
#' @param L1_precomp,L2_precomp optional precomputed sample Laplacians
#' @param La1_precomp,La2_precomp optional precomputed anchor submatrices of Laplacians
...
#' fit <- woven_v2_dual(X1, X2, anchor_idx = anchor_idx, Y = Y, K = K)
#' dim(fit$W1)
#' @export
woven_v2_dual <- function(X1, X2, anchor_idx, Y,
    ...
    if (verbose) {
        message(sprintf(
            "WOVEN V=2 dual | n=%d, n_a=%d, p1=%d, p2=%d, K=%d, gamma_y=%.2f",
    ...
    if (!is.null(La1_precomp)) {
        ...
        if (verbose) message("  Building Laplacians on observed subjects (no imputation)...")
    ...
    if (verbose) message("  Computing M matrix square roots (n_a x n_a)...")
    sq1 <- mat_sqrt(M1)
    sq2 <- mat_sqrt(M2)
    ...
    if (verbose) message("  Computing SVD of dual P (n_a x n_a)...")
    P_dual <- sq1$inv_sqrt %*% IKY %*% sq2$inv_sqrt

    K_use <- min(K, n_a - 1L, nrow(P_dual), ncol(P_dual))
    if (K_use < K) {
        warning(sprintf("K reduced %d -> %d.", K, K_use))
        K <- K_use
    }
    ...
    if (verbose) message("  Recovering W from dual solution...")
    Ka1 <- tcrossprod(Xa1)
    Ka2 <- tcrossprod(Xa2)
