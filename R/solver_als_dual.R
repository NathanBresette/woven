...
woven_als_dual <- function(X_list, anchor_idx, Y,
  ...
  if (verbose) cat("  Extracting anchor Laplacians...\n")
  ...
  if (verbose) cat("  Factorizing M matrices (n_a x n_a)...\n")
  ...
  m_orth <- function(Z, R_M) {
    ...
    if (!is.null(R2)) return(t(backsolve(R2, t(Z))))
  ...
  warm_start <- function() {
    ...
    lapply(seq_len(V), function(v) {
      Z0 <- if (.restart_count == 1L) Xa_svd_U[[v]] else matrix(rnorm(n_a * K), n_a, K)
  ...
  als_run <- function(Z_init) {
    ...
    for (iter in seq_len(max_iter)) {
      ...
      if (is.finite(obj_prev) &&
        ...
        obj_trace <- obj_trace[seq_len(iter)]; break
      }
      obj_prev <- obj
    }
  ...
  for (r in seq_len(n_restarts)) {
    if (verbose) cat(sprintf("  Restart %d/%d...\n", r, n_restarts))
  ...
  if (verbose) cat("  Recovering W_list from dual solution...\n")
