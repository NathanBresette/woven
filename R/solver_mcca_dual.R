...
woven_mcca_dual <- function(X_list, anchor_idx, Y,
  ...
  if (verbose) cat("  Computing M square roots (n_a x n_a)...\n")
  ...
  MiIKY <- lapply(seq_len(V), function(v)
    sq_list[[v]]$inv_sqrt %*% IKY)           # M_v^{-1/2} IKY, n_a x n_a
  ...
  if (verbose) cat("  Eigendecomposition...\n")
  ...
  if (verbose) cat("  Extracting Za and recovering W...\n")
